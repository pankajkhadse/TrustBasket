...
declare global {
  interface Window {
    selectLocation?: (id: any) => void;
  }
}

export default function LocationMap({ locations, selectedLocation, onLocationSelect }: { locations: any[], selectedLocation?: any, onLocationSelect?: any }) {
  ...
  useEffect(() => {
    if (mapRef.current) {
      ...
      window.selectLocation = (locationId: any) => {
    ...
    <div className="w-full">
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
        <div className="bg-green-50 p-3 rounded-lg">
          <div className="text-lg font-bold text-green-600">{locations.filter((l: any) => l.type === "vendor").length}</div>
          <div className="text-sm text-gray-600">Vendors</div>
        </div>
        <div className="bg-orange-50 p-3 rounded-lg">
          <div className="text-lg font-bold text-orange-600">
            {locations.filter((l: any) => l.type === "supplier").length}
          </div>
          <div className="text-sm text-gray-600">Suppliers</div>
        </div>
        <div className="bg-blue-50 p-3 rounded-lg">
          <div className="text-lg font-bold text-blue-600">
            {(locations.reduce((sum: any, l: any) => sum + l.rating, 0) / locations.length).toFixed(1)}
          </div>
